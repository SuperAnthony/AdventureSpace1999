...
namespace WhatPumpkin.CutScenes {
	...
	public class CutScene : Keyed, IKeyed, IPerform, IPlayable {
		...
		/// <summary>
		/// The sound effect that gets played when this cutscene is played.
		/// </summary>

		[SerializeField] SFX _soundEffect;
		...
		public void Play() {
			...
			// Handle sound
			// Set the sound effect audio source
			try {

				if (_soundEffect.AudioClip != null) {
					_soundEffect.AudioSource = (SgridAudioSource)GameController.CutsceneManager.CutSceneAudioSource;
				}
			}
			catch (InvalidCastException invalidCastExeption){
			
				throw(invalidCastExeption);

			}

			// Stop the music (TODO: Will want to change this to a fade out)
			// Perhaps a method to fade out all sounds would be useful here
			GameController.SoundController.StopAllMusic (); // TODO: Temp
			// Play the sound clip associated with it

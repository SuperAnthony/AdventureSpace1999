...
public class AIPath : MonoBehaviour, IAIPath {

	#region Anthony Added
...
//		set { transform = value; }
	...
	// ** What Pumpkin - Anthony ** //
	// Gets the current velocity based on it's desired velocity from the calculated velocity
	public Vector3 CurrentVelocity { get { return CalculateVelocity (GetFeetPosition ()); } }
	...
	public virtual void Update () {
		...
		if (Mathf.Abs (CurrentVelocity.x + CurrentVelocity.y + CurrentVelocity.z) > 0) {
						//Rotate towards targetDirection (filled in by CalculateVelocity)
						RotateTowards (targetDirection);
				} 
		else {
			/*
			this.transform.rotation = new Quaternion(target.transform.rotation.x,

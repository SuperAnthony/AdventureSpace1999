...
namespace WhatPumpkin.Sgrid.Markers { 
	...
	public class SpawnPoint : Marker, IKeyed, ISpawnPoint, IActivatable {
		...
		[SerializeField] bool _fadeToBlack = true;
		...
		void HandleCameraFadeEnd() {
		
			Spawn (); 
			CameraTransitions.fadeToBlackComplete -= HandleCameraFadeEnd;
		}
		...
		void Spawn(bool playActionSequence = true) {
			...
			if (GameController.PartyManager != null && GameController.PartyManager.ActivePC != null) {
						GameController.PartyManager.ActivePC.GetComponent<AIPath> ().Disable ();
				}
			...
			if (GameController.PartyManager != null && GameController.PartyManager.ActivePC != null) {
						GameController.PartyManager.ActivePC.GetComponent<AIPath> ().Enable ();
		...
		/// <summary>
		/// Activate this instance. Give the option to not play the action sequence. 
		/// </summary>
		
		private void Activate(bool playActionSequence) {
		...
		void ActivateChildSpawnPoints() {
			...
			foreach (SpawnPoint spawnPoint in _childSpawnPoints) {
				...
				if(spawnPoint != null){spawnPoint.Activate(false);}
